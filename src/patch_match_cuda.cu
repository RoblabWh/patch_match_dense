	...
	for (int r = 0; r < 2; r++) {
		...
		for (int k1 = -patchhalf; k1 <= patchhalf; k1+=2) {
			...
			for (int k2  = -patchhalf; k2 <= patchhalf; k2+=2) {
				x = j+k2;
				y = i+k1;
				I_src = tex2D<float>(d_texs[0], x+0.5f, y+0.5f);
				...
				//ComputeScore_Push(I_src, I_tgt, &sum_x, &sum_y, &sum_xx, &sum_yy, &sum_xy, &sum_w);
...
void ComputeBearingsEquirect(float* d_Bearingmap, const int width, const int height) {
	...
	if (err != cudaSuccess)
	{
		std::cout << "ComputeBearingsEquirect " << cudaGetErrorString(err) << std::endl;
	}
}

void ComputeRandomPlanemapDepthmap(float* d_Depthmap, float* d_Planemap,float*  d_Bearingmap, int width, int height, float min_depth, float max_depth, curandState* d_StatesMap) {
	...
	if (err != cudaSuccess)
	{
		std::cout << "ComputeRandomPlanemapDepthmap " << cudaGetErrorString(err) << std::endl;
	}
}

void ComputeRandomPlanemap(float* d_Planemap, float* d_Depthmap, float* d_Bearingmap, int width, int height, curandState* d_StatesMap) {
	...
	if (err != cudaSuccess)
	{
		std::cout << "ComputeRandomPlanemap " << cudaGetErrorString(err) << std::endl;
	}

}

void ScorePlaneDepth(const int patchhalf, float* d_Scoremap, float* d_Depthmap, float* d_Planemap, float* d_Bearingmap, cudaTextureObject_t* d_texs, int width, int height) {
	...
	if (err != cudaSuccess)
	{
		std::cout << "ScorePlaneDepth " << cudaGetErrorString(err) << std::endl;
	}
}

void PatchMatchRedBlackPass(const int patchhalf, int nIterations, float* d_Scoremap, float* d_Depthmap, float* d_Planemap, float* d_Bearingmap, cudaTextureObject_t* d_texs, int width, int height, curandState* d_StatesMap) {
	...
	if (err != cudaSuccess)
	{
		std::cout << "PatchMatchRedBlackPass " << cudaGetErrorString(err) << std::endl;
	}
...
void ComputeRandomSeed(curandState* d_StatesMap, const int width, const int height) {
	...
	if (err != cudaSuccess)
	{
		std::cout << "ComputeRandomSeed " << cudaGetErrorString(err) << std::endl;
	}
}

void RandomInitialization(const int patchhalf, float* d_Scoremap, float* d_Depthmap, float* d_Planemap, float* d_Bearingmap,
